<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold bg-gradient-to-r from-white to-gray-200 bg-clip-text text-transparent">Thêm loại sản phẩm mới</h1>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div>
          ...
          <textarea 
            ...
            rows="4" 
            class="block w-full bg-gray-700/50 border border-gray-600 text-white rounded-lg shadow-sm py-3 px-4 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent placeholder-gray-400"
            placeholder="Nhập mô tả cho loại sản phẩm này"
          ></textarea>
        </div>
...
const submit = async () => {
  ...
  try {
    ...
    toast.success('Loại sản phẩm đã được tạo thành công!')
    
    router.push('/admin/categories')
  } catch (e) {
    const toast = useToast()
    toast.error(e.data?.message || 'Không thể tạo loại sản phẩm', 'Lỗi')
    console.error(e)
  } finally {
    loading.value = false
  }
