<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold bg-gradient-to-r from-white to-gray-200 bg-clip-text text-transparent">Quản lý Loại sản phẩm</h1>
      <NuxtLink 
...
const loadCategories = async () => {
  try {
    const response = await $fetch(`${config.public.apiBase}/api/admin/categories`, {
...
const deleteCategory = async (id) => {
  ...
  const confirmed = await confirmDelete('loại sản phẩm này')
  if (!confirmed) return
  ...
  try {
    ...
    const toast = useToast()
    toast.error('Không thể xóa loại sản phẩm', 'Lỗi')
    console.error(e)
  }
}
