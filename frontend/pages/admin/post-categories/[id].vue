<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold bg-gradient-to-r from-white to-gray-200 bg-clip-text text-transparent">Chỉnh sửa loại tin tức</h1>
...
const loadCategory = async () => {
  try {
    const data = await $fetch(`${config.public.apiBase}/api/admin/post-categories/${route.params.id}`, {
    ...
    toast.error('Không tìm thấy loại tin tức', 'Lỗi')
...
const submit = async () => {
  loading.value = true
  try {
    await $fetch(`${config.public.apiBase}/api/admin/post-categories/${route.params.id}`, {
      method: 'PUT',
      body: form,
      headers: { Authorization: `Bearer ${token.value}` }
    })
    ...
    const toast = useToast()
    toast.success('Loại tin tức đã được cập nhật thành công!')
    
    router.push('/admin/post-categories')
  } catch (e) {
    const toast = useToast()
    toast.error(e.data?.message || 'Không thể cập nhật loại tin tức', 'Lỗi')
    console.error(e)
  } finally {
    loading.value = false
  }
